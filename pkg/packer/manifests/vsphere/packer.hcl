...
locals {
  ...
  distro_version_bootfile_lookup = {
   ...
   distro_boot_command_lookup = {
     "RHEL-7" = local.el_bootcommand
     "RHEL-8" = local.el_bootcommand
     "CentOS-7" = local.el_bootcommand
     "CentOS-8" = local.el_bootcommand
     "Ubuntu" = [""]
     "Debian" = [""]
    }

  bootfile_name = lookup(local.distro_version_bootfile_lookup, "${var.distro_name}-${var.distro_version}", "")
  bootfile = lookup(local.bootfiles_reg, local.bootfile_name , "")
  boot_command = lookup(local.distro_boot_command_lookup, "${var.distro_name}-${var.distro_version}", [""])
}
